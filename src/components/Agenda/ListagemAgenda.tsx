...
import { AgendaInterface } from '../../interfaces/AgentaInterface';
import axios from 'axios';
import { Link } from 'react-router-dom';
import { CadastroProfissionalInterface } from '../../interfaces/CadastroProfissionalInterface';
import Header from '../Servico/HeaderServico';
import Swal from 'sweetalert2';
import Footer from '../Servico/FooterServico';
...
const ListagemAgenda = () => {
    ...
    const buscar = (e: FormEvent) => {
        ...
        async function fetchData() {
            try {
                ...
                const response = await axios.post('http://127.0.0.1:8000/api/agenda/procurarAgenda',
                    {
    ...
    const excluir = (id: number)=>{
        async function fetchData(){
            try{
                const response = await axios.delete('http://127.0.0.1:8000/api/agenda/excluirAgenda/'+ id);
                if(response.data.status === true){

                    const response = await axios.get('http://127.0.0.1:8000/api/agenda/retornarTodos/');
                    setAgendas(response.data.data);
